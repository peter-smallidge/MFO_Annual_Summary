...
report_raw <- read_csv("data_reported_2022_2023.csv") %>% 
  #remove duplicates
  group_by(county, reside_zip, owner_email) %>% 
  slice_head(n=1) %>% 
  ungroup


request_raw <- read_csv("data_requested_2022_2023.csv") %>% 
  #remove duplicates
...
tot_request <- request_raw %>% 
  ...
  summarize(
    ...
    Number_Requests = n(),
    Counties_With_Requests = n_distinct(county_name, na.rm = T)
  ) 
...
tot_reports <- report_raw %>% 
  ...
  summarize(
    ...
    Number_of_Reports = n(),
    Counties_With_Reports = n_distinct(county, na.rm = T)
  ) 
...
table01 <- full_join(residence_pct, table01a,  by = join_by(region)) %>% 
  mutate(
    Reports_Per_Request = Number_of_Reports /Number_Requests *100
  ) %>% 
  select(region, Number_Requests, Wooded_Acres_Requested,Counties_With_Requests, 'Live_on_Property_(pct)',
         Number_of_Reports,Wooded_Acres_Reported,Counties_With_Reports,Reports_Per_Request,
        ) %>% 
    mutate_if(is.numeric, round, 0)
...
county_count <- request_raw %>% 
  arrange(region, county_name) %>% 
  group_by(region, county_name) %>% 
  summarize(
    n = n(),
    AVG_acres = mean(wooded_acres, na.rm=T),
    TOT_acres = sum(wooded_acres, na.rm=T),
...
table11a <- pivot_longer(admin_format, c(
...
mgmt_format <- report_raw %>% 
  rename(
     ...
     FVM_Chemical = action_mgmt_fvm,
     FVM_Mechanical = action_mgmt_insect,
...
```{r}
volunteer01 <- read_csv("data_reported_2022_2023.csv") %>%  
  select(mfo1, mfo2, mfo3, county)
...
dfs2xlsx(withNames("state wide metrics request", table01,
                   ...
                   "vol names", vol_names #no comma after last df name, before paren 
                         ), "summary_of_data_R_output_2022_2023.xlsx")
